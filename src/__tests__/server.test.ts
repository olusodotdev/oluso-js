import { RateLimiter, getServerContext } from '../utils/server';
...
describe('getServerContext', () => {
    it('returns process/host metadata', () => {
        const context = getServerContext();

        expect(context.hostname).toEqual(expect.any(String));
        expect(context.nodeVersion).toBe(process.version);
        expect(context.processId).toBe(process.pid);
        expect(context.memory.used).toBeGreaterThan(0);
